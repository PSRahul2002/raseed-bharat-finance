...
class Item(BaseModel):
    ...
    category: Optional[str] = Field(description="Main category of the item")
    subcategory: Optional[str] = Field(description="Subcategory of the item")
...
2. Assign a suitable `category` and `subcategory` to each item based on its description.
3. Determine a `bill_category` for the entire receipt based on the most common category among the items. If there's a mix, choose the most relevant or use "Mixed".
4. Match the category to one of these predefined options:
...
Subcategory can be derived from the item description (e.g. "Zomato order" -> Food -> Delivery, or "Indigo flight" -> Travel -> Flights).
...

{
  ...
  "taxes": 40.00,
  "items": [
    {"name": "Logo design", "quantity": 1, "unit_price": 200.0, "category": "Electronics", "subcategory": "Digital Services"},
    {"name": "Website design", "quantity": 1, "unit_price": 120.0, "category": "Electronics", "subcategory": "Web Development"}
  ],
...
def process_receipt_json_with_gemini(receipt_data):
    ...
    try:
        ...
        1. Assign a suitable `category` and `subcategory` to each item based on its description.
        2. Determine a `bill_category` for the entire receipt based on the most common category among the items. If there's a mix, choose the most relevant or use "Mixed".
        3. Match the category to one of these predefined options:
        ...
        Return only a valid JSON with all the original data plus the category information.
        """
